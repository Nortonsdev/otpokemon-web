 ...
 */
/**
 * Create an ItemNeedsCountFn from a classic item catalog (stackable / fluid).
 */
export function createItemNeedsCountFromCatalog(
  catalog: Map<number, { stackable?: boolean; fluid?: boolean }>,
): ItemNeedsCountFn {
  return (id: number): boolean => {
    const e = catalog.get(id)
    return !!(e?.stackable || e?.fluid)
  }
}
